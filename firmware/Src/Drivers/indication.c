...
{
	led_ptr->hardware.mode = INDICATION_LED_LOADING;
...
void indication_led_button_setup(mculed_t *led_ptr, int led_index)
{
	switch (led_index) {
		case LED_TOP:
			...
			led_ptr->setup.on_ms = (INDICATION_LED_SPEED_FAST * LED_TOP_TIME_ON);
			led_ptr->setup.off_ms = (INDICATION_LED_SPEED_FAST * LED_TOP_TIME_OFF);
